...
function saveProductToServer(name, protein, carbs, fats, ccal) {
	...
	fetch('https://6725230fc39fedae05b40910.mockapi.io/fat_Alien/calories', {
		...
		.then(data => {
			...
			// Обновляем отображение калорий после сохранения
			document.getElementById('totalMyCcal').textContent = `${ccal} ккал`;
		})
		.catch(error => {
...
document.getElementById('saveButton').addEventListener('click', () => {
	...
	// Сохраняем данные на сервер и обновляем отображение калорий
